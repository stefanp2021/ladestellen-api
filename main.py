        ...
        if(df_Station_from_Operator.empty == True):
            ...
            for m in range(df_Station_from_Operator.shape[0]):
                ...
                if(df_get_iter_item_dfStation[PLZ].isdigit() and not df_get_iter_item_dfStation[Ort].isdigit()):
                    ...
                    print("-----------")
                    fehler = "problem beim upload to SQLAdmin"
                #print("----------------------------------------------------------------------------------------")


    print(" Finished Station API request: The Station request from the API is finished")
except:
    msg = "Problem bei den Ladestellen, Fehler = ".format(fehler)
    
    sendMail(sender="linux@muenzer.at",subject="Ladestellen-Problem", recipient=["david.kohlfuerst@muenzer.at","stefan.pirker@muenzer.at"], username="",password="",message=msg)
